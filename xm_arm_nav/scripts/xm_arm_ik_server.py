...
class XmArmIkServer():
    def __init__(self):
        ...
        self.arm_joint = ['joint_lifting','joint_waist','joint_big_arm','joint_fore_arm','joint_wrist','joint_small_arm']
        # strange error when using the arm_tolerance in the data_check function
        self.arm_tolerance = {'max':[0.09,1.57,1.57,2.10,2.20,1.57],
                              'min':[-0.18,-0.35,-1.57,-2.10,-2.20,-1.57]}
    ...
    def call_back(self,req):
        ...
        if req.goal.header.frame_id is None:
            ...
            # only need the first 6 elements
            current_joint_state = current_joint_state[:6]
