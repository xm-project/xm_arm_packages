...
class ArmDirect(State):
    ...
    def execute(self,userdata):
        ...
        if service_bool ==False :
            ...
            ik_req = xm_SolveIKRequest()
            ik_req.goal = deepcopy(userdata.arm_ps)
            #grasp_joints = deepcopy(userdata.arm_ps)
            ik_res = self.ik_client.call(ik_req)
            ik_bool =True
            self.ik_client.close()
            if ik_res.result ==True:
...
class xm_arm_smach():
    def __init__(self):
        ...
        self.sm_ArmStack = StateMachine(outcomes=['succeeded','aborted','error'],
                                            input_keys=['arm_point','arm_mode','objmode'])
        ...
        with self.sm_ArmStack:
            ...
            StateMachine.add('ARM_DIRECT',
                                ArmDirect(),
                                transitions={'succeeded':'AFTER_GRASP','aborted':'aborted','error':'error'},
                                remapping={'gripper_mode':'arm_mode','arm_ps':'arm_point','objmode':'objmode'})
    ...
    def callback(self,req):
        ...
        self.objmode = req.objmode
        sm_result = self.sm_ArmStack.execute(parent_ud ={'arm_point':self.arm_point,'arm_mode':self.arm_mode,'objmode':self.objmode})
