...
class ObstaclesGen:
    def __init__(self):
        ...
        rospy.init_node('obstacles_gen')#初始化
        # move group
        xm_arm_ = moveit_commander.MoveGroupCommander("xm_arm")#使用晓萌的机械臂
        self.end_effector_link =xm_arm_.get_end_effector_link()#设置末端执行器
        print self.end_effector_link#打印末端执行器
        self.scene_interface = moveit_commander.PlanningSceneInterface()#实例化PlanningSceneInterface对象，这个接口围绕机器人的世界
        self.scene_pub = rospy.Publisher('planning_scene', PlanningScene, queue_size=5)
        self.colors = dict()
        rospy.sleep(0.6)
        # planningsceneinterface
        self.scene_interface.remove_attached_object(self.end_effector_link, name = None)#最开始先移除所有的物体
        self.scene_interface.remove_world_object(name = None)#remove all thing when starting
        self.obstacles_pub = rospy.Service('xm_obstacles',xm_Obstacles, self.service_callback)#生成障碍物的服务器
        rospy.spin()
        # log out
