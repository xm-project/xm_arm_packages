...
private:
	...
	bool service_callback(xm_msgs::xm_PickOrPlaceRequest &req,xm_msgs::xm_PickOrPlaceResponse &res)//"xm_pick_or_place"
	{
		...
		if(req.action==1)//pick
		{
			...
			std::vector<double> joint_value(6);
	...
	bool prepare_nav()
	{
		...
		ROS_INFO("fuck");
		bool success = (bool)xm_arm_.plan(my_plan);
		if (!success)
			return false;
	...
	bool execute(const std::vector<double> &goal)//向hardware发送数据
	{
		if(goal.size()!=6)
		{
			ROS_ERROR("Can not execute the num of goal is not 6");
			return false;
		}
		for(int i=0 ; i<6 ;i++) std::cout<<"Execute"<<goal[i]<<" ";
		std::cout<<std::endl;
		// 执行目标关节值
		moveit::planning_interface::MoveGroup xm_arm_("xm_arm");
	...
	bool gripper_exe(bool mod)
	{
		...
		bool success = (bool)xm_gripper_.plan(my_plan);
